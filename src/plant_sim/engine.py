...
class PlantSimulation:
    def __init__(
        ...
        self.flow = FlowTracker() if track_flow else None
    ...
    def _init_stage_resources(self) -> None:
        ...
        scan = stages.scan_in
        if scan.workers and hasattr(scan.workers, "normal") and scan.workers.normal:
            self._scan_resources["normal"] = simpy.Resource(
                self.env, capacity=scan.workers.normal
            )
        if scan.workers and hasattr(scan.workers, "reduced") and scan.workers.reduced:
            self._scan_resources["reduced"] = simpy.Resource(
                self.env, capacity=scan.workers.reduced
            )
        if not self._scan_resources:
            n = scan.worker_count()
    ...
    def _run_stage(
        ...
        if stage_name == "scan_in":
            pool = self._scan_resources.get(
                worker_mode, self._scan_resources["normal"]
            )
        else:
    ...
    def _inject_items(self, count: float) -> None:
        ...
        for i in range(n):
            self.env.process(self._item_pipeline(i))
        if remainder > 0.01 and self.rng.random() < remainder:
            self.env.process(self._item_pipeline(n))
    ...
    def run(self) -> SimulationResult:
        ...
        drain_until = self.env.now + 24 * 60 * 12
